  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-24 right-6 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center z-50 transition-all duration-300 transform 
          ${isOpen 
            ? 'bg-gray-800 dark:bg-gray-700 text-white rotate-90' 
            : 'bg-gradient-to-tr from-brand-600 to-purple-600 text-white hover:shadow-brand-500/50 hover:scale-105 fab-pulse'
          }`}
      >
        {isOpen ? <X size={24} /> : <MessageSquare size={28} className="animate-[pulse_2s_infinite]" />}
      </button>

      {/* Chat Panel */}
      <div 
        className={`fixed bottom-40 right-6 w-[90vw] md:w-[380px] max-h-[600px] h-[70vh] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden z-40 border border-gray-200 dark:border-gray-800 transition-all duration-300 cubic-bezier(0.16, 1, 0.3, 1) origin-bottom-right backdrop-blur-md bg-opacity-95 dark:bg-opacity-95
          ${isOpen 
      ...
      >
        ...
        <div className="p-4 bg-gradient-to-r from-brand-600 to-purple-600 flex justify-between items-center shrink-0 shadow-md z-10">
          ...
          <button 
            ...
            className="text-white/80 hover:text-white transition-colors rotate-hover rounded-full p-1 hover:bg-white/10"
          >
            <X size={20} />
          </button>
        </div>
