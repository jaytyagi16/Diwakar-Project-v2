...
import { ShieldAlert, AlertTriangle, UserCheck, Sparkles, ArrowRight, FileText, Car, CreditCard, Hash, Loader2 } from 'lucide-react';
...
export const Step2_Triage: React.FC<Step2Props> = ({ onNext, fnolData, setTriageData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (fnolData) {
        try {
            const result = await aiService.generateTriage(fnolData);
            setData(result);
        ...
        // Fallback if accessed directly without FNOL (shouldn't happen in normal flow)
        const dummy = generateDummyData();
        setData(dummy.triage);
        setTriageData(dummy.triage);
      }
    ...
    fetchData();
  }, [fnolData, setTriageData]);

  if (loading) {
    return (
        <div className="flex flex-col items-center justify-center h-[50vh] space-y-4">
            <Loader2 className="w-10 h-10 text-indigo-500 animate-spin" />
            <p className="text-slate-500">Analyzing claim details and calculating risk scores...</p>
        </div>
    );
  }
